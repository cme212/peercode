 ...
 public:
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    size_type index() const {
      // HW0: YOUR CODE HERE
      return graph_->nodes_[id_].node_id_;
    }
    ...
    size_type degree() const {
      return graph_->adj_list[id_].size();
    }
  ...
  class Edge : private totally_ordered<Edge> {
   public:
    ...
    bool operator<(const Edge& e) const {
      //HW0: YOUR CODE HERE
      size_type n1 = std::min(node1_id_, node2_id_);
      size_type e_n1 = std::min(e.node1_id_, e.node2_id_);

      size_type n2 = std::max(node1_id_, node2_id_);
      size_type e_n2 = std::max(e.node1_id_, e.node2_id_);

      return std::tie(graph_, n1, n2) < std::tie(graph_, e_n1, e_n2);
    }
  ...
  Edge edge(size_type i) const {
    // HW0: YOUR CODE HERE
    return Edge(this, edge_list[i][0], edge_list[i][1]);        
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    // HW0: YOUR CODE HERE
    for (size_type i = 0; i < adj_list[a.id_].size(); i++) {
      if (b.id_ == adj_list[a.id_][i])
        return true;
    }
    return false;
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (!(has_edge(a, b))) {
      adj_list[a.id_].push_back(b.id_);
      adj_list[b.id_].push_back(a.id_);
      std::vector<size_type> pair {a.id_, b.id_};
  ...
  class NodeIterator : private totally_ordered<NodeIterator>{
   public:
    ...
    bool operator==(const NodeIterator& node_iter) const {
      return (graph_ == node_iter.graph_) && (id_ == node_iter.id_);
    }
   ...
   private:
    ...
    Graph* graph_;
    size_type id_;
    NodeIterator(Graph* g, size_type id) : graph_(g), id_(id) {}
  };
  ...
  class IncidentIterator : private totally_ordered<IncidentIterator>{
   public:
    ...
    // Supply definitions AND SPECIFICATIONS for:
    Edge operator*() const {
      size_type node2_id_ = graph_->adj_list[node_id_][inc_id_];
      return Edge(graph_, node_id_, node2_id_);
    }
  ...
  class EdgeIterator : private totally_ordered<EdgeIterator>{
   public:
    ...
    EdgeIterator& operator++() {
      ++inc_iter;
      size_type node_id_ = node_iter.id_;
      size_type inc_id_ = inc_iter.inc_id_;
      while ((inc_iter == (*node_iter).edge_end()) || (node_id_ > graph_->adj_list[node_id_][inc_id_])) {
        if (inc_iter == (*node_iter).edge_end()) {
          ++node_iter;
          inc_iter = (*node_iter).edge_begin();
        } else {
          ++inc_iter;
        }
        node_id_ = node_iter.id_;
        inc_id_ = inc_iter.inc_id_;
        if (node_id_ == graph_->nodes_.size())
          break;
      } 
      return *this;
    }
 ...
 private:
  ...
  struct internal_node {
    ...
    internal_node(const Point& point, size_type id, node_value_type val)
      :  point(point), node_id_(id), value(val) {}
  };
  ...
  std::vector<std::vector<size_type> > edge_list;
};
