...
class Graph {
 ...
 public:
  ...
  Graph() 
    ...
    : Nodes_p(), edge_n1(), edge_n2(),node_m(), size_n(0), size_e(0), val_type(), adj_n(){
  }
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    // return value
    node_value_type& value(){
        return graph_p->val_type[node_id];
    }
    ...
    // calculate degree
    size_type degree() const{
        return (graph_p->adj_n[node_id]).size();
    }
    // incident iterator
    incident_iterator edge_begin() const{
    ...
    incident_iterator edge_end() const{
        size_type adj_e_size = (graph_p->adj_n[node_id]).size();
  ...
  class Edge : private totally_ordered<Edge> {
   public:
    ...
    Node node1() const {
      ...
      /************ Added code ************/
      if(node_pivot >= 0){
          if((int) (*(graph_e->edge_n1[edge_id])).index()  == node_pivot){
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if(Graph::has_node(a) && Graph::has_node(b) && (a == b) == false){
        ...
        if(Graph::has_edge(a,b) == false){
            ...
            /************ Added Code HW1*******/ 
            if(adj_n.count(a.index()) > 0){
                adj_n[a.index()].push_back(size_e);
            } else {
                std::vector<unsigned> tmp{size_e};
                adj_n.insert({a.index(),tmp});
            }
            if(adj_n.count(b.index()) > 0){
                adj_n[b.index()].push_back(size_e);
            } else {
                std::vector<unsigned> tmp{size_e};
                adj_n.insert({b.index(),tmp});
            }
            size_e++;
            return new_edge;
        } else {
  ...
  class IncidentIterator : private totally_ordered<IncidentIterator>{
   public:
    ...
    Edge operator*() const{
        std::vector<size_type> tmp = graph_p->adj_n[node_inc];
