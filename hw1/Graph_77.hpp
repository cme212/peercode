 ...
 private:
  ...
  // Interal type for nodes:
	 ...
	 Point intpoint_; //Point of node
         ...
         internal_node(Point intpoint, unsigned intnidx, V intval) 
               : intpoint_(intpoint), intnidx_(intnidx), intval_(intval) {}
      };
...
//
  ...
  Node add_node(const Point& position, const node_value_type& value = node_value_type()) {
    ...
    nodes_vec.emplace_back(internal_node(position, size_, value)); //modified to add value 
  ...
  class NodeIterator : private totally_ordered<NodeIterator> {
   public:
    ...
    Node operator*() const {
        ...
        internal_node intnode = it_graph_->nodes_vec.at(it_idx_);
        unsigned idx = intnode.intnidx_; 
        return Node(it_graph_,idx); 
    }
    NodeIterator& operator++(){
        // Increment iterator index and return instance
        
        if(it_idx_ < it_graph_-> size_){
            it_idx_ = it_idx_ + 1;
        }
        return *this; 
    }
  ...
  class EdgeIterator : private totally_ordered<EdgeIterator> {
   public:
    ...
    Edge operator*() const {
        ...
        internal_edge intedge = evec_ptr_->at(eit_idx_);
        unsigned idx = intedge.inteidx_; //Get edge index
        return Edge(eit_graph_, idx, intedge.intnode1idx_, intedge.intnode2idx_);
    }
   ...
   private:
    ...
    // Index of the iterator
    unsigned eit_idx_;
    // Pointer to the vector of internal edges
    const std::vector<internal_edge>* evec_ptr_;
    // Private constructor:
    EdgeIterator(const Graph* eit_graph, const std::vector<internal_edge>* evec_ptr, unsigned eit_idx)
    :eit_graph_(const_cast<Graph*>(eit_graph)),evec_ptr_(evec_ptr),eit_idx_(eit_idx) {}
  };
  ...
  // Supply definitions AND SPECIFICATIONS for:
   edge_iterator edge_begin() const {
       ...
       return EdgeIterator(this, &edges_vec, 0);
   }
   edge_iterator edge_end() const {
       // Return iterator instance pointing to back of edge vector
       return EdgeIterator(this, &edges_vec, edges_vec.size());
   }
