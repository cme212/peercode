 ...
 */
template <typename V> //Suddenly not passing HW0
class Graph {
 private:
 ...
 public:
  ...
  /** Modifying graph to become class template */
  ...
  class Node : private totally_ordered<Node> { // Proxy Node actually
   public:
    ...
    //Node() {} //Default initializing uid_ and parent_graph_
    //Default initializing a size_type & default graph pointer 
    // in a local scope leads to undefined value!
  ...
  class IncidentIterator : private totally_ordered<IncidentIterator> {
   public:
    ...
    Edge operator*() const{ //return this edge
      //std::pair<size_type, size_type> edge_ba = {(*incident_iter_).uid_, root_node_.uid_};
   ...
   private:
    ...
    //Iterates through the vector of adjacent nodes
