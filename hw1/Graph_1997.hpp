...
#include <vector>
#include <cassert>
#include <utility> //allows use of pairs for edges...went away from idea
...
class Graph {
 ...
 public:
  ...
  struct neighborNode; //forward declare
  struct internalNode; //forward declare
  struct internalEdge; //forward declare
  ...
  /*I realized I needed a better structure for the relationship between
  adjecent nodes while making the incident iterator for HW1.
  Looked through peer code and found several examples (pairs seemed like
  the most common, also maps with key value pairings). I have modeled
  mine off of Graph_1038 and am citing it as my source for the following
  struct to aid in iteration implementation.
  */
  struct neighborNode {
  ...
  struct internalNode {
      ...
      //Constructor
      internalNode(Point p, size_type index, size_type val, std::vector<neighborNode> neighvec) {
          nodepos = p;
          nodeID = index;
          value = val;
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    Node() {
      // HW0: YOUR CODE HERE
      //this seems sufficient for now...should I instead return an error?
    }
  ...
  //end of public Graph class methods for edges
  ...
  struct internalEdge {
      ...
      size_type edgeID;
      //Constructor
      internalEdge(Node a, Node b, size_type index) {
          A = a;
          B = b;
          edgeID = index;
      }
  ...
  class Edge : private totally_ordered<Edge> {
   public:
    ...
    Edge() {
      //this seems sufficient similar to Node()
    }
     ...
     */
    bool operator<(const Edge& e) const {
      if (mainGraph == e.mainGraph && edgeindex < e.edgeindex) {
          return true;
      }
      return false;
    }
   ...
   private:
    ...
    size_type edgeindex;
    //constructor to allow Graph class to construct node objects
    Edge(const graph_type* graph, size_type indexID) {
        mainGraph = const_cast<graph_type*>(graph);
        edgeindex = indexID;
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    edgevec.emplace_back(a, b, nextvectorindex);
  ...
  /*This code contains the correct logic and works logic but is way too slow.
   *I ran it for all three cases, but the large file takes 2 minutes.
   *My intial thought was to check if has_edge is false, but that requires
   *going through the whole edgevec each time, and multiple times if true.
   *So I figured instead go through the same has_edge logic in this function
   *stop if the edge already exists. So atmost one iteration through edgevec.
   *Still too slow. Too late for HW0 to change my data structure to a map
   *but if the nodes were keys, I could get O(1) look up.
   */
  /*Edge add_edge(const Node& a, const Node& b) {
    for (size_type i=0; i<edgevec.size(); i++) {
        if ((edgevec[i].A == a && edgevec[i].B == b) ||
            (edgevec[i].B == a && edgevec[i].A == b)) {
            return Edge(this, i-1);
        }
    }
    size_type nextvectorindex;
    nextvectorindex = totedges;
  ...
  class NodeIterator : private totally_ordered<NodeIterator> {
   public:
    ...
    NodeIterator& operator++() {
        ...
        //check that its not at the last element...-1?
  ...
  class IncidentIterator : private totally_ordered <IncidentIterator>{
   public:
    ...
    Edge operator*() const {
        ...
        return Edge(iIterGraph, edgeID);
    }

    IncidentIterator& operator++() {
        //move to the next neaighbor node on the 'spoke'
        spokeNode++;
        return *this;
    }
  ...
  class EdgeIterator : private totally_ordered<EdgeIterator> {
   public:
    ...
    Edge operator*() const {
        ...
        size_type indexref;
        indexref = eIterGraph->edgevec[eIterIndex].edgeID;
        return Edge(eIterGraph, indexref);
    }

    EdgeIterator& operator++() {
        //size_type iterGraphsize;
        //iterGraphsize = eIterGraph->num_nodes();
        //check that its not at the last element
        //if (eIterIndex < iterGraphsize) {
            eIterIndex++;
        //}
        //passed by reference and thus was modified, return the same iterator
        return *this;
