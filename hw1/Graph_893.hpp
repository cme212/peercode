...
class Graph {
 ...
 public:
  ...
  class Node : private totally_ordered<Node> {
   ...
   private:
     ...
     * */
    Node(const graph_type* graph_, size_type index_) : graph_(graph_),
                                                        index_(index_) {
  ...
  node_type add_node(const Point& position, 
    ...
    node_type n = this->node(this->next_node_index_++);
  ...
  class NodeIterator : private totally_ordered<NodeIterator> {
   public:
    ...
    node_type operator*() const {
      ...
      // iterator. 
      return node_type(this->graph_, this->it_->first);
    }
  ...
  class IncidentIterator : private totally_ordered<IncidentIterator> {
   public:
    ...
    edge_type operator*() const {
      ...
      node_type n = this->graph_->node(node_id);
   ...
   private:
     ...
     * iterator.
     */
    IncidentIterator(const Graph* graph_, const node_type node_,
 ...
 private:
  ...
  struct internal_node {
    ...
    internal_node(const Point position) : position(position), value() {}

    internal_node(const Point position, V value) : position(position), 
                                                   value(value) {}
...
template <typename V>
constexpr typename Graph<V>::size_type Graph<V>::Node::INVALID_ID = \
                                                    Graph<V>::size_type(-1);
template <typename V>
constexpr typename Graph<V>::size_type Graph<V>::Edge::INVALID_ID = \
                                                    Graph<V>::size_type(-1);
#endif
