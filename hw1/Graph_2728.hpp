...
class Graph {
 ...
 public:
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    size_type degree() const {
      return graph_->neighbor_map_[index()].size();
    }
  ...
  Node add_node(const Point& position, const node_value_type& value = node_value_type()) {
    ...
    nodes_.emplace_back(position, uid, value);
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (edges_.count(check_edge) < 1) {
      ...
      // make edge tuple to map to true
      edges_[check_edge] = true;
  ...
  node_iterator node_end() const { return NodeIterator(this, nodes_.size()); }
  ...
  class IncidentIterator : private totally_ordered<IncidentIterator> {
   public:
    ...
    bool operator==(const IncidentIterator& it) const {
      return (graph_== it.graph_ && **this == *it);
    }
 ...
 private:
  ...
  struct node_struct {
    ...
    node_struct(const Point point_, size_type uid_, node_value_type value_) 
      : point(point_), uid(uid_), value(value_) { }
  };
  ...
  // edge map
  std::map<std::tuple<size_type, size_type>,bool> edges_;
