...
class Graph {
 ...
 public:
   ...
   * @param[in] node The node object to be removed
   * @post new num_nodes() == old num_nodes() - 1
   * @post has_node(node) == false
   * @return 1 if node exists and successfully removed, 0 otherwise
   *
   * Complexity: O((max degree between two nodes)^2)
   */
  size_type remove_node(const Node& node){
    // remove all incident edges
    //for(auto ei = node.edge_begin(); (++ei)!=node.edge_end(); ei = node.edge_begin()){
    while(node.degree()>0){
      this->remove_edge(*node.edge_begin());
    }
    size_type swapped_uid = current_nodes[node_num-1];
    size_type removed_node_id = node.index();
    // node doesn't exist if node.index() >= num nodes, return 0
    if(removed_node_id >= node_num){
      return 0;
    }
    // move the target node to the last in current_nodes and pop
    current_nodes[removed_node_id] = swapped_uid;
    current_nodes.pop_back();
    // change the index of the swapped edge
    nodes_container[swapped_uid].proxy_id = removed_node_id;
    node_num--;
    return 1;
  }
  /** Remove a node from the graph
   * @param[in] n_it The iterator to a node object to be removed
   * @post new num_nodes() == old num_nodes() - 1
   * @post has_node(node) == false
   * @return graph.node_begin()
   *
   * Complexity: O((max degree between two nodes)^2)
   */
  node_iterator remove_node(node_iterator n_it){
    Node removing_n = *n_it;
    this->remove_node(removing_n);
  ...
  class Edge : private totally_ordered<Edge>{
   public:
    ...
    edge_value_type& value() {
      return graph_->edges_container.at(graph_->current_edges.at(id_)).value;
    }
   ...
   * @param[in] edge The edge object to be removed
   * @post new num_edges() == old num_edges() - 1
   * @post has_edge(edge.node1(), edge.node2()) == false
   * @return 1 if edge exists and successfully removed, 0 otherwise
   *
   * Complexity: O(max degree of the two connecting nodes) 
   */
  size_type remove_edge(const Edge& edge){
    size_type swapped_uid = current_edges[edge_num-1];
    size_type removed_edge_id = edge.index();
    // edge doesn't exist if edge.index() >= num edges, return 0
    if(removed_edge_id >= edge_num){
      return 0;
    }
    size_type node1uid = current_nodes[edge.node1().index()];
    size_type node2uid = current_nodes[edge.node2().index()];
    // remove edge from node1 and node2's adjacent node list
    for(auto c_iter = nodes_container[node1uid].connecting_nodes.begin(); 
      c_iter != nodes_container[node1uid].connecting_nodes.end(); ++c_iter){
      if((*c_iter).node2_uid == node2uid){
        nodes_container[node1uid].connecting_nodes.erase(c_iter);
        break;
      }
    }
    for(auto c_iter = nodes_container[node2uid].connecting_nodes.begin(); 
      c_iter != nodes_container[node2uid].connecting_nodes.end(); ++c_iter){
      if((*c_iter).node2_uid == node1uid){
        nodes_container[node2uid].connecting_nodes.erase(c_iter);
        break;
      }
    }

    // move the target edge to the last in current_edges and pop
    current_edges[removed_edge_id] = swapped_uid;
    current_edges.pop_back();
    // change the index of the swapped edge
    edges_container[swapped_uid].proxy_id = removed_edge_id;
    edge_num--;
    return 1;
  }

  /** Remove an edge from the graph
   * @param[in] node1 node object on one end of the edge
   * @param[in] node2 node object on the other end of the edge
   * @post new num_edges() == old num_edges() - 1
   * @post has_edge(node1, node2) == false
   * @return 1 if edge exists and successfully removed, 0 otherwise
   *
   * Complexity: O(max degree of the two connecting nodes) 
   */
  size_type remove_edge(const Node& node1, const Node& node2){
    int edge_uid = check_edge_uid(node1,node2);
    if(edge_uid == -1){
      return 0;
    }
    Edge removing_e = this->edge(edges_container[edge_uid].proxy_id);
    return this->remove_edge(removing_e);
  }

  /** Remove an edge from the graph
   * @param[in] e_it edge iterator pointing to the edge object to be removed
   * @post new num_edges() == old num_edges() - 1
  ...
  edge_iterator remove_edge(edge_iterator e_it){
    Edge removing_e = *e_it;
    this->remove_edge(removing_e);
 ...
 private:
  ...
  struct internal_edge{
    ...
    internal_edge(size_type pid, size_type id1, size_type id2) : 
      proxy_id(pid), node1_uid(id1), node2_uid(id2){} // edge value is not initialized
  };
