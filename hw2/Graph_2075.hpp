 ...
 public:
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    node_value_type& value() { return graph_->nodes_[uindex_]->value; }
    ...
    incident_iterator edge_begin() const {
      return IncidentIterator(graph_, uindex_,
                              graph_->nodes_[uindex_]->incident.begin());
    }
  ...
  class Edge : private totally_ordered<Edge> {
   public:
    ...
    edge_value_type& value() { return graph_->edges_[uindex_]->value; }

    const node_value_type& value() const {
      return graph_->edges_[uindex_]->value;
    }
    ...
    Point vector() const { return node1().position() - node2().position(); }
    ...
    bool operator==(const Edge& e) const {
      // HW0: YOUR CODE HERE
      return graph_ == e.graph_ && uindex_ == e.uindex_;
    }
   ...
   private:
    ...
    Edge(const graph_type* graph, size_type uindex, size_type node1_uindex,
  ...
  Edge edge(size_type i) const {
    ...
    return Edge(this, edge_uindex, (edges_[edge_uindex])->node1_uindex,
  ...
  Edge add_edge(const Node& a, const Node& b,
    ...
    // HW0: YOUR CODE HERE
    assert(a != b);
    auto this_edge = make_pair_ordered(a.uindex_, b.uindex_);
    auto tmp = has_edge_.find(this_edge);
    if (tmp != has_edge_.end())
      return Edge(this, tmp->second, a.uindex_, b.uindex_);
    size_type next_edge_uindex = edges_.size();
    edges_.push_back(new Edge_{a.uindex_, b.uindex_, value});
    ...
    num_edges_++;
    return Edge(this, next_edge_uindex, a.uindex_, b.uindex_);
  }
  ...
  class IncidentIterator : private equality_comparable<IncidentIterator> {
   public:
    ...
    // Edge operator*() const
    Edge operator*() const {
      return Edge(graph_, incident_iter_->second, node_uindex_,
                  incident_iter_->first);
    }
    ...
    bool operator==(const IncidentIterator& i) const {
      return graph_ == i.graph_ && node_uindex_ == i.node_uindex_ &&
             incident_iter_ == i.incident_iter_;
    }
  ...
  size_type remove_edge(const Node& n1, const Node& n2){
    auto this_edge = make_pair_ordered(n1.uindex_, n2.uindex_);
    auto tmp = has_edge_.find(this_edge);
    if (tmp == has_edge_.end()) return 0;
    edge_index_to_uindex_.erase(std::lower_bound(edge_index_to_uindex_.begin(),edge_index_to_uindex_.end(),tmp->second));
    nodes_[n1.uindex_]->incident.erase(n2.uindex_);
    nodes_[n2.uindex_]->incident.erase(n1.uindex_);
    has_edge_.erase(tmp);
    num_edges_--;
    return 1;
  }
  ...
  size_type remove_node(const Node& n){
    ...
    while(n.degree()) remove_edge(*n.edge_begin());
    index_to_uindex_.erase(index_to_uindex_.begin()+n.index());
    for(size_type i=n.index();i<index_to_uindex_.size();i++) nodes_[index_to_uindex_[i]]->index=i;
    size_--;
    return 1;
  }
