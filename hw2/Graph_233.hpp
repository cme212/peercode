...
private:
  ...
  std::vector<edge_value_type> edge_value_collection;
...
//          return Point();
    ...
    Point& position() {
      // HW2: YOUR CODE HERE
      // TX: added the code without exception handling
    ...
    */
    ...
    size_type degree() const {
        // HW1: YOUR CODE HERE
        size_type deg_count = 0;
        for (size_type idx = 0; idx < graph_ptr->num_edges(); idx++){
            size_type node_1 = graph_ptr->edge_collection[idx][0];
            size_type node_2 = graph_ptr->edge_collection[idx][1];
            bool cond = (node_1 == node_idx) || (node_2 == node_idx);
            if (cond ) {
                deg_count += 1;
            }
        }
    ...
    size_type graph_size() const {
        // HW2: added graph_size() function for node
        assert_GRAPH_PTR_EXIST_TX;
        return graph_ptr->point_collection.size();
    }
    ...
    bool is_valid_node() const {
        ...
        bool cond2;
        if (cond1) {
            cond2 = node_idx < size_type(graph_ptr->size());
        }
        else{
            cond2 = false;
        }
    ...
    void set_value(node_value_type v) {
        (graph_ptr)->set_value(node_idx, v);
    }
  ...
  size_type remove_node(const Node & v){
      if (!v.is_valid_node()){
          // Do nothing
          return 0;
      }
          ...
          std::vector<std::array<size_type, 2> > e_vec_tmp;
          for (auto iter = v.edge_begin(); iter != v.edge_end(); ++iter){
              Edge e_tmp = *iter;
              e_vec_tmp.push_back({e_tmp.node1().index(), e_tmp.node2().index()});
          }

          for (auto iter = e_vec_tmp.begin(); iter != e_vec_tmp.end(); ++iter){
              remove_edge(node((*iter)[0]), node((*iter)[1]));
          }

          // Modify edge_index value
          size_type node_idx = v.index();

          for (size_type iter = 0; iter < edge_collection.size(); iter++){
              auto index_tuple = edge_collection[iter];
              for (size_type idx = 0; idx <= 1; idx++){
                  if (index_tuple[idx] > node_idx){
                      index_tuple[idx] = index_tuple[idx] - 1;
                  }
              }
              edge_collection[iter] = index_tuple;
          }

          // Remove node
          point_collection.erase(point_collection.begin() + node_idx);
          node_value_collection.erase(node_value_collection.begin() + node_idx);
          return 1;
      }
  }
  ...
  void set_value_edge(size_type i, edge_value_type v) {
    assert (i < this->size());
      edge_value_collection.at(i) = v;
  }
  ...
  private:
    ...
    */
    ...
    edge_value_type& value(){
        return graph_ptr->edge_value_collection[edge_idx];
    };
  ...
  size_type num_edges_set() const {
    return (this->edge_collection_set).size();
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    if (!a.is_valid_node() || !b.is_valid_node()){
      ...
      for (size_type idx = 0; idx < this->num_edges(); idx++){
  ...
  //   size_type node_idx_a = a.node_idx;
  //   size_type node_idx_b = b.node_idx;
  //   auto edge_tmp = edge_custom_type({node_idx_a, node_idx_b});
  //   auto count_result = this->edge_collection_set.count(edge_tmp);
  //   if (count_result>0){
  //       return true;
  //   }
  //   else{
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // HW0: YOUR CODE HERE
      ...
      // HW2 edition: initialize null value and add to collection
      edge_value_type tmp_value {};
      (this->edge_value_collection).push_back(tmp_value); // Add the point

      return this->edge(num_edges() - 1);
  }

  Edge add_edge(const Node& a, const Node& b, const edge_value_type& tmp_value) {
    // HW0: YOUR CODE HERE
      // TX: this block of code is from has edge
      Edge e = add_edge(a, b);
      set_value_edge(e.edge_idx, tmp_value);
      return e;
  }
  ...
  size_type remove_edge(const Node& a, const Node& b){
      ...
      if (has_edge){
          edge_collection.erase(edge_collection.begin() + edge_idx);
          edge_value_collection.erase(edge_value_collection.begin() + edge_idx);
          this->edge_collection_set.erase(edge_custom_type({node_idx_a, node_idx_b}));
          this->edge_collection_set.erase(edge_custom_type({node_idx_b, node_idx_a}));
          return 1;
      }
      else{
          return 0;
    ...
    Edge operator*() const {
      ...
      EdgeIterator& operator++() {
          ...
          if (edge_idx != -1){
              ...
              if (edge_idx >= int((*this->graph_ptr).num_edges() )){
                  edge_idx = -1; // Becomes null iterator
              }
          }
          return *this;
  ...
  //HW2: remove edge&node: YOUR CODE HERE
