 ...
 public:
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    Point& position(){
      return const_cast<Point &>(static_cast<const Node &>(*this).position());
    }
    ...
    bool operator<(const Node& n) const {
      ...
      if (graph_!=n.graph_){ return (long)graph_<(long)n.graph_;}
      return uid_<n.uid_;
    }
  ...
  Node add_node(const Point& position, const node_value_type& value=node_value_type()) {
    ...
    std::map<size_type, edge_value_type> edge_vals;
    e_value_map.push_back(edge_vals);
  ...
  class Edge : private totally_ordered<Edge> {
   public:
    ...
    const edge_value_type& value() const {
	    return (graph_->e_value_map[uid1_]).at(uid2_);
    }
     ...
     */
    bool operator<(const Edge& e) const {
      if (graph_!=e.graph_){ return (long)graph_<(long)e.graph_;}
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (!has_edge(a,b)){
      ...
      edge_value_type value=norm(a.position()-b.position());
      e_value_map[a.uid_].insert({b.uid_,value});
      e_value_map[b.uid_].insert({a.uid_,value});
    }
 ...
 *         0 if the node does not exist in the graph
 * @pre    @a n used to be a valid node of the graph
 * @complexity O(num_nodes) assuming num_indicent_egde is much less than num_nodes
 * @post   the node represented by @a n is completely removed from the graph
 *         any node iterator pointing to that node is still valid but will point to another
 *         node or become equivalent to @new graph.node_end(). 
 *         node iterators poingting to the last node will become equivalent to @new graph.node_end()
 *         @old graph.node_end() will become invalid
 *         graph.size() decrease by 1
 */

  size_type remove_node(const Node& n){
    if (!validity[n.uid_]){return 0;}
    validity[n.uid_]=false;
    while (!look_up[n.uid_].empty()){
      remove_edge(n, Node(this, look_up[n.uid_][0]));
    }
    size_type old_size=indices.size();
 ...
 * @param[in] b      Another node of the edge
 * @return 1 if the edge is successfully removed
 *         0 if the edge does not exist
 * @pre    @a a, @a b are valid nodes of the graph
 * @complexity O(num_edges)
 * @post   the edge represented by {@a a, @a b} is completely removed from the graph
 *         any edge iterator pointing to that edge is still valid but will point to another
 *         edge or become equivalent to @new graph.edge_end(). 
 *         edge iterators poingting to the last edge will become equivalent to @new graph.edge_end()
 *         @old graph.edge_end() will become invalid
 *         graph.num_edges decrease by 1
 */
  
  size_type remove_edge(const Node& a, const Node& b){
    //remove edge indices
    size_type old_size=edge_indices.size();
    size_type i=0;
    for(; i<old_size; i++){
      if ((edge_indices[i][0]==a.uid_ && edge_indices[i][1]==b.uid_) || \
          (edge_indices[i][1]==a.uid_ && edge_indices[i][0]==b.uid_)){
        edge_indices[i]=edge_indices.back();
        edge_indices.pop_back();
        break;
      }
    }
    if (i==old_size){ return 0;}
    //remove connectivity info
    for(size_type i=0; i<look_up[a.uid_].size(); i++){
      if (look_up[a.uid_][i]==b.uid_){
        look_up[a.uid_][i]=look_up[a.uid_].back();
        look_up[a.uid_].pop_back();
        break;
      }
    }
    for(size_type i=0; i<look_up[b.uid_].size(); i++){
      if (look_up[b.uid_][i]==a.uid_){
        look_up[b.uid_][i]=look_up[b.uid_].back();
        look_up[b.uid_].pop_back();
        break;
      }
    }
    //remove value info
    e_value_map[a.uid_].erase(b.uid_);
    e_value_map[b.uid_].erase(a.uid_);
    return 1;
  }

/** works the same as remove_edge(const Node& a, const Node& b)
 *  except it accepts an Edge object as input
 */
  size_type remove_edge(Edge& e){
    return remove_edge(e.node1(),e.node2());
  }
/** works the same as remove_edge(const Node& a, const Node& b)
 *  except it accepts an EdgeIterator object as input and returns the same EdgeIterator,
 *  which points to another edge in the graph
 */
 ...
 private:
  ...
  std::vector<std::map<size_type,edge_value_type> > e_value_map; //maps index to values
};
