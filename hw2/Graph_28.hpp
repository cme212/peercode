...
public:
  ...
  public:
    ...
    bool operator<(const Node &n) const
    {
      bool graph_eq = true; // (graph_ == n.graph());
    ...
    Point spring_force() const
    {
      Point force;
      for (auto first = edge_begin(); first != edge_end(); ++first)
      {
        force -= ((norm((*first).length()) - (*first).value()) * (*first).k_const() * (*first).length()) / norm((*first).length());
      }
      return force;
    }
  ...
  void change_position(Node n, Point point)
  {
    auto index = n.index();
    internal_nodes_[index].point = point;
  }
  ...
  {
  public:
    edge_value_type &value() { return value_; };
    const edge_value_type &value() const { return value_; };
    ...
    bool operator<(const Edge &e) const
    {
      if (index_node1_ < e.node1().index())
        return true;
      else if (index_node1_ == e.node1().index() && index_node2_ < e.node2().index())
        return true;
    ...
    edge_value_type k_const() { return k_const_; }
 ...
 */
  ...
  Edge add_edge(const Node &a, const Node &b)
  {
    ...
    else
    {
      ...
      num_edges_++;
      edge_num = num_edges_;
      adjacency_matrix_[ordered_pair.first][ordered_pair.second] = edge_num;
    }
  ...
  size_type remove_edge(const Edge &edge)
  {
    ...
    if (!has_edge(edge.node1(), edge.node2()))
      return 0;
    auto ordered_pair = order_nodes(edge.node1(), edge.node2());
    auto edge_idx = adjacency_matrix_[ordered_pair.first][ordered_pair.second];
    adjacency_matrix_[ordered_pair.first][ordered_pair.second] = 0;
    for (unsigned int i = 0; i < adjacency_matrix_.size(); i++)
    {
      for (unsigned int j = 0; j < adjacency_matrix_[i].size(); j++)
      {
        if (adjacency_matrix_[i][j] > edge.index_ + 1)
        {
    ...
    for (unsigned int i = edge_idx + 1; i < num_edges(); i++)
    {
      edges_[i] = edges_[i + 1];
    }

    edges_.erase(num_edges_);
    num_edges_--;
  ...
  size_type remove_edge(const Node &node1, const Node &node2)
  {
    ...
    {
      auto x = get_edge(node1, node2);
      Edge y = Edge(this, node1.index(), node2.index(), x);
      return remove_edge(y);
    }
    else
      return 0;
  }
  ...
  size_type remove_node(const Node &node)
  {
    if (!has_node(node))
      return 0;
    for (auto first = node.edge_begin(); first != node.edge_end(); ++first)
      remove_edge(*first);
    internal_nodes_.erase(internal_nodes_.begin() + node.index_);
    if (adjacency_matrix_.size() > node.index_)
      adjacency_matrix_.erase(adjacency_matrix_.begin() + node.index_);
    for (unsigned int i = 0; i < adjacency_matrix_.size(); i++)
    {
      if (adjacency_matrix_[i].size() > node.index_)
        adjacency_matrix_[i].erase(adjacency_matrix_[i].begin() + node.index_);
    }
