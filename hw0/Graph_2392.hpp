...
class Graph {
 ...
 public:
  ...
  bool has_node(const Node& n) const {
    return (nodes_.find(n.index()) != nodes_.end()) && (n.graph_ == this);
    // HW0: YOUR CODE HERE
    // (void) n;            // Quiet compiler warning
    // return false;
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for (size_type i = 0; i < edges_.size(); i++) {
      if ((edges_.at(i).node1() == a && edges_.at(i).node2() == b) ||
          (edges_.at(i).node1() == b && edges_.at(i).node2() == a)) {
        return true;
      }
    }
    return false;
 ...
 private:
  std::unordered_map<int, node_type> nodes_;
