...
class Graph {
 ...
 public:
  ...
  class Node {
   public:
    ...
    size_type index() const {
      ...
      auto id_equal = [this] (const InternalNode node) 
                      { return (node.id == this->id); };

      auto node_it = std::find_if((**graph).nodes.begin(), 
                                  (**graph).nodes.end(), 
                                  id_equal); 

      size_type index = node_it - (**graph).nodes.begin(); 
      return index;  
    }
   ...
   private:
    ...
    const InternalNode& fetch() const {
  ...
  Node add_node(const Point& position) {
    // HW0: YOUR CODE HERE
    nodes.emplace_back(position, next_id);
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    bool edge_found = edges_present.count(std::make_tuple(a.id,b.id))
                      or edges_present.count(std::make_tuple(b.id,a.id));
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (not has_edge(a, b)) {
      ...
      edges_present.emplace(a.id,b.id); 
    } 
 ...
 private:
  ...
  std::set<std::tuple<size_type,size_type>> edges_present;  
  ...
  struct InternalNode {
    ...
    InternalNode(const Point& position, size_type id) : position(position), 
                                                        id(id) {} 
  };
