...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    ...
    nodes_.push_back(new Node_(size_, position));
 ...
 private:
  ...
  struct Node_ {
    ...
    Node_(size_type index, Point position) : index(index), position(position) {}
  };
  ...
  struct Edge_ {
    ...
    Edge_(size_type node1_uindex, size_type node2_uindex)
        : node1_uindex(node1_uindex), node2_uindex(node2_uindex) {}
  };

  std::vector<Edge_*> edges_;
  std::vector<size_type> edge_index_to_uindex_;
  std::map<std::pair<size_type, size_type>, size_type> has_edge_;
