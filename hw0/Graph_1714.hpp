...
class Graph {
 ...
 private:
  ...
  std::unordered_map<size_type, std::vector<size_type> > adj_list;
 ...
 public:
  ...
  bool has_node(const Node& n) const {
    ...
    return nodes.find(node_index) != nodes.end();
  }
 ...
 public:
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    if (adj_list.find(a.uid_) != adj_list.end()){
      std::vector<size_type> a_edges = adj_list.at(a.uid_);
      if (std::find(a_edges.begin(), a_edges.end(), b.uid_) != a_edges.end()){
        return true;
      }
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if(new_edge){
      ...
      edge.node1 = a;
      edge.node2 = b;
      edge.uid = edge_next_uid_;
      edges.insert(std::make_pair(edge_next_uid_, edge));

      // add to adj list for both nodes
      adj_list.at(a.uid_).push_back(b.uid_);
      adj_list.at(b.uid_).push_back(a.uid_);
      ...
      return Edge(this, edge_next_uid_);
    }
  }
