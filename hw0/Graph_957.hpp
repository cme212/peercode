...
private:
  ...
  std::map<std::set<unsigned>, unsigned> e2ID;
...
public:
  ...
  Node add_node(const Point &position)
  {
    // HW0: YOUR CODE HERE
    node_size_ += 1;
    proxyNode proxynode = proxyNode(node_size_ - 1, position);
  ...
  bool has_edge(const Node &a, const Node &b) const
  {
    ...
    std::set<size_type> node_set = {a.id, b.id};
  ...
  {
    // HW0: YOUR CODE HERE
    std::set<size_type> key = {a.id, b.id};
    if (has_edge(a, b))
    {
    ...
    edge_size_ += 1;
    e2ID[key] = edge_size_ - 1;
    proxyEdge proxyedge = proxyEdge(a.id, b.id, edge_size_ - 1);
    Edges.push_back(proxyedge);
    return Edge(this, edge_size_ - 1); // Invalid Edge
  }
...
private:
  ...
  struct proxyNode
  {
    size_type index;
    Point pt;
    proxyNode(size_type index, Point pt)
    {
      this->index = index;
      this->pt = pt;
    }
