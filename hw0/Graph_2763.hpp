  ...
  class Node {
   public:
    ...
    const Point& position() const {
      // HW0: YOUR CODE HERE
      return *(graph_->node_vector[node_idx_]);
    }
  ...
  class Edge {
   public:
    ...
    bool operator==(const Edge& e) const {
      //HW0: YOUR CODE HERE
      bool nodes = (node1_idx_ == e.node1_idx_ && node2_idx_ == e.node2_idx_);
      return (nodes && (graph_ == e.graph_) && (edge_idx_ == e.edge_idx_));
    }
...
//            graph_(const_cast<graph_type*>(graph)), edge_idx_(eid){}
    Edge(const graph_type* graph, size_type idx1, size_type idx2, size_type eid):
            graph_(const_cast<graph_type*>(graph)), node1_idx_(idx1), node2_idx_(idx2),edge_idx_(eid){}
  };
  ...
  Edge edge(size_type i) const {
    // HW0: YOUR CODE HERE
    assert(i < num_edges());
    size_type idx_a = edge_vector[i][0];
    size_type idx_b = edge_vector[i][1];
    return Edge(this, idx_a, idx_b, i);
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for (size_type i = 0; i < num_edges(); i++){
      if(edge_vector[i][0] == node_a and edge_vector[i][1] == node_b){
        return true;
      }
      else if (edge_vector[i][0] == node_b and edge_vector[i][1] == node_a) {
        return true;
      }
    }
    return false;
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    edge_vector.push_back({a.node_idx_, b.node_idx_});
    return Edge(this, a.node_idx_, b.node_idx_, num_edges()-1);
  }
