...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    ...
    internal_node node = internal_node{position, connected};
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    std::unordered_set <size_t> a_nodes_ = nodes_.at(a_idx).connected_nodes_;
    
    // return is a and b has been connected
    if (a_nodes_.find(b_idx) != a_nodes_.end()) {
      return true;
    } else {
      return false;
    };
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (dont_add) {
      ...
      while (!found && e != edges_.end()) {
        i = e->first;
        internal_edge edge = e->second;
