...
class Graph {
 ...
 public:
  ...
  class Node {
   public:
    ...
    Node() {
      // HW0: YOUR CODE HERE
      Graph::node_type x; // constructing invalid node
    }
  ...
  Node add_node(const Point& position) {
    ...
    internal_node new_node = internal_node(position, new_id);
  ...
  class Edge {
   public:
    ...
    bool operator==(const Edge& e) const {
      //HW0: YOUR CODE HERE
      assert(e._graph_obj == _graph_obj);
      return (e.node1() == node1() && e.node2() == node2()) || \
      (e.node1() == node2() && node2() == node1());
    }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    std::set<size_type> compare_set; compare_set.insert(a._node_id); \
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    std::set<size_type> new_set; new_set.insert(a._node_id); new_set.insert(b._node_id);
    if (!has_edge(a, b)) { 
 ...
 private:
  ...
  internal_node(const Point& position, \
    const size_type idx) : _position(position), _idx(idx) {};
  };
