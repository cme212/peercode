...
#include <vector>
#include <cassert>
#include <utility> //allows use of pairs for edges
...
class Graph {
 ...
 public:
  ...
  struct internalNode {
      ...
      size_type nodeID;
      //Constructor
      internalNode(Point p, size_type index) {
          nodepos = p;
          nodeID = index;
      }
  };
  ...
  Node add_node(const Point& position) {
    ...
    //as arguments
    nodevec.emplace_back(position, nextvecindex);
    totnodes++;
    //(void) position;      // Quiet compiler warning
    return Node(this, nodevec.size()-1); //didn't like mainGraph
  }
