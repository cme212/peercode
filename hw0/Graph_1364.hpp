...
class Graph {
 private:
  ...
  std::map<std::set<unsigned int>,unsigned int> Etoi; 
  // HW0: YOUR CODE HERE
 ...
 public:
  ...
  Node add_node(const Point& position) {
    ...
    size_++;
    nodeProxy new_node = nodeProxy(position,size_-1);
  ...
  class Edge {
   public:
    ...
    Node node1() const {
      ...
      return Node(graph,nodeId);      // Invalid Node
    }
   ...
   private:
    ...
    Edge(const Graph* g,unsigned int i){
      this->graph = g;
      this->id = i;
    };
  };
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // HW0: YOUR CODE HERE
    (void) a, (void) b;   // Quiet compiler warning
    std::set<unsigned int> key1,key2;
    key1.insert(a.ind);
    key1.insert(b.ind);
    key2.insert(b.ind);
    key2.insert(a.ind);
    if (has_edge(a,b)){
      ...
      edge_size++;
      edgeProxy new_edge = edgeProxy(a.ind,b.ind,edge_size-1);
