...
class Graph {
 private:
  ...
  std::vector<std::pair<unsigned, unsigned>> edges; // store the edges
 ...
 public:
  ...
  Edge edge(size_type i) const {
    ...
    return Edge(edges[i].first, edges[i].second, i, this);
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    // check whether pair(a, b) is in the edges vector
    std::pair<size_type, size_type> pair_idx = std::make_pair(a.idx, b.idx);
    if (find(edges.begin(), edges.end(), pair_idx) != edges.end())
      return true;

    // check whether pair(b, a) is in the edges vector
    pair_idx = std::make_pair(b.idx, a.idx);
    if (find(edges.begin(), edges.end(), pair_idx) != edges.end())
      return true;
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    // check whether pair(a,b) belongs to the edges, if yes, return the edge
    std::pair<size_type, size_type> pair_idx = std::make_pair(a.idx, b.idx);
    auto it = find(edges.begin(), edges.end(), pair_idx);
    if (it != edges.end()){
      size_type index = it - edges.begin();
      return Edge(a.idx, b.idx, index, this);
    }

    // check whether pair(b,a) belongs to the edges, if yes, return the edge
    pair_idx = std::make_pair(b.idx, a.idx);
    it = find(edges.begin(), edges.end(), pair_idx);
    if (it != edges.end()){
      size_type index = it - edges.begin();
      return Edge(b.idx, a.idx, index, this);
    }

    // add the new edge to the graph
    Edge add_edge = Edge(a.idx, b.idx, numEdges, this);
    pair_idx = std::make_pair(a.idx, b.idx);
    edges.push_back(pair_idx);
    numEdges++;
